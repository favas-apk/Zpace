...
import com.project.milan.interfac.Fr_CartInterface;
import com.project.milan.pojos.read_web_rate_and_offers.DetailsItem;
import com.project.milan.pojos.read_web_rate_and_offers.Response;
...
import com.project.milan.activity.PaymentDirectActivity;
import com.project.milan.adapter.AdapterCart;
import com.project.milan.adapter.AdapterDeliveryAds;
import com.project.milan.custom.PlayEditTextView;
...
public class Fragment_Cart extends Fragment implements Fr_CartInterface {
    ...
    private List<com.project.milan.pojos.states.DetailsItem> list_states = new ArrayList<>();
    private CardView card1;
    private List<com.project.milan.pojos.save_del_ads.DetailsItem> list_del_ads;
    ...
    public void add_del_address() {
        ...
        com.project.milan.pojos.states.Response pojo = new com.project.milan.pojos.states.Response();
        Gson gson = new Gson();
        pojo = gson.fromJson(jsonString, com.project.milan.pojos.states.Response.class);


        list_states.addAll(pojo.getDetails());

        for (com.project.milan.pojos.states.DetailsItem row : list_states) {
        ...
        btnNext.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (edtname.getText().toString().trim().equals("") || edtmob.getText().toString().trim().equals("") || edtpin.getText().toString().trim().equals("") || edtads.getText().toString().trim().equals("") || edtarea.getText().toString().trim().equals("") || edtcity.getText().toString().trim().equals("")
                    ...
                    Call<com.project.milan.pojos.save_del_ads.Response> call = apiService.save_del_addres(Constants.api_key, Utils.GVCOT(db.getLoginEntityDao().get_customer_id().get(0)), Utils.GVCOT(edtname.getText().toString().trim()), Utils.GVCOT(edtmob.getText().toString().trim()), Utils.GVCOT(edtpin.getText().toString().trim()), Utils.GVCOT(edtads.getText().toString().trim()), Utils.GVCOT(edtarea.getText().toString().trim()), Utils.GVCOT(edtlandmark.getText().toString().trim()), Utils.GVCOT(edtcity.getText().toString().trim()), Utils.GVCOT(spn1.getSelectedItem().toString().trim()));

                    call.enqueue(new Callback<com.project.milan.pojos.save_del_ads.Response>() {
                        @Override
                        public void onResponse(Call<com.project.milan.pojos.save_del_ads.Response> call, retrofit2.Response<com.project.milan.pojos.save_del_ads.Response> response) {
                            ...
                            if (response.body() != null) {
                                if (response.body().getResult().equals("1")) {
                                    ...
                                    for (com.project.milan.pojos.save_del_ads.DetailsItem row : list_del_ads) {
                        ...
                        @Override
                        public void onFailure(Call<com.project.milan.pojos.save_del_ads.Response> call, Throwable t) {
    ...
    private void save_order_two(String del_slno) {
        ...
        if (!db.getLoginEntityDao().get_customer_id().get(0).toString().trim().equals("")) {
            ...
            if (send_size > 0) {
                List<DetailsItem> list_web_rate_and_offers = new ArrayList<>();
                list_stock_prob_items = new ArrayList<>();
                whole_stockid = "(" + whole_stockid + ")";
                Endpoint apiService = ApiClient.getClient().create(Endpoint.class);

                Call<Response> call = apiService.save_order(Constants.api_key, wholejsonObject.toString(), send_size, whole_stockid);
                    ...
                    public void onResponse(Call<Response> call, retrofit2.Response<Response> response) {
                        ...
                        if (response.body() != null) {
                            if (response.body().getResult().equals("1")) {
                                ...
                                for (DetailsItem row : list_web_rate_and_offers) {
