...
import com.project.milan.pojos.search_box.Response;
...
public class Fragment_SearchBefore extends Fragment implements Fr_SearchBefore_Interface {
    ...
    private List<DetailsItem> list_search = new ArrayList<>();
...
//            }
            ...
            Call<Response> call = apiService.read_item_for_search_box(Constants.api_key);
...
//                    Log.i("speech", "result: " + result);
    ...
    public void delete(String item) {
        ...
        for (DetailsItem row : list_search) {
            pos = pos + 1;
    ...
    private void set_the_adapter()
    {
        ...
        if (db.getInvEntityDao().get_count()>10) {
            ...
            for (String item : db.getSearchEntityDao().get_all_datas()) {
                ...
                list_search.add(new DetailsItem(item, true));

            }

            for(String item:db.getInvEntityDao().get_all_datas())
            {
                list_search.add(new DetailsItem(item, false));
            }
